use crate::ml::activation::CalcMod;

use super::nl::NeuralLayer;
use super::sample::Sample;
use super::matrix::Matrix;
use super::matrix::MatrixTrait;
use super::cost::CostFunction;
use super::cost::squared_error::SquaredError;
use super::utils::sample_input_to_matrix;
use super::utils::sample_output_to_matrix;
...
impl NeuralNetwork {
    ...
    pub fn forward(&self, sample: &Sample,calc_mode: &CalcMod) -> Vec<Matrix> {
        if self.layers.len() == 0 {
        ...
        for (i, layer) in self.layers.iter().enumerate() {
            ...
            if i > 0 {
                let mult: Matrix = prev_weight
                    ...
                    .map_row(&|n| layer.activation.calc(n,calc_mode));
    ...
    pub fn evaluate(&self, sample: &Sample,calc_mode: &CalcMod) -> Matrix {
        let forward: Vec<Matrix> = self.forward(sample,calc_mode);

        forward.last().cloned().unwrap()
    }
    ...
    /// backpropagation
    pub fn train(&mut self, mut samples: Vec<Sample>, epochs: i32, learning_rate: f64,calc_mode: CalcMod) {
        for _ in 0..epochs {
            let mut_samples = samples.as_mut_slice();

            let mut error_value = vec![];

            for sample in mut_samples.iter() {

                let mut output: Vec<Matrix> = self.forward(&sample,&calc_mode);
                ...
                for (i, layer) in output.iter().enumerate() {
                    ...
                    let forward_derivative: Matrix =
                        layer.map_row(&|n| self.layers[index].activation.derivative(n,&calc_mode));
...
mod tests {
    ...
    // use crate::ml::activation::SoftMax;
    // use crate::ml::activation::HyperbolicTangent;
    // use crate::ml::sample::Sample;
    // use crate::ml::nl::NeuralLayer;
    // use crate::ml::nn::NeuralNetwork;
    // use crate::ml::matrix::MatrixTrait;
    // use crate::ml::cost::cross_entropy::CrossEntropy;
